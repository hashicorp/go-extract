...
func TestIs7zip(t *testing.T) {
	...
	for _, tt := range tests {
		if got := extract.Is7zip(tt.header); got != tt.want {
...
func TestUnpack7zip(t *testing.T) {
	...
	tc := []struct {
		...
		},
		{
			...
			generator:   createSimpleReader,
			c:           extract.NewConfig(extract.WithCacheInMemory(true)),
			expectError: false,
		},
		{
			name:        "unpack 7zip, input size limit",
			generator:   createByteReader,
			c:           extract.NewConfig(extract.WithMaxInputSize(25)),
			expectError: true,
		},
		{
			name:        "unpack 7zip, output size limit",
			generator:   createByteReader,
			c:           extract.NewConfig(extract.WithMaxExtractionSize(5)),
			expectError: true,
		},
		{
	...
	for _, tt := range tc {
		t.Run(tt.name, func(t *testing.T) {
			...
			// check config
			if tt.c == nil {
				tt.c = extract.NewConfig()
			}

			// unpack archive
			err := extract.Unpack7Zip(context.Background(), testingTarget, tmpDir, r, tt.c)
